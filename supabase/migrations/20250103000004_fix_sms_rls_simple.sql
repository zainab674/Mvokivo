-- Fix SMS RLS policy to work without conversations table
-- Drop existing policies
DROP POLICY IF EXISTS "Users can view SMS messages they are involved in" ON sms_messages;
DROP POLICY IF EXISTS "Users can insert SMS messages" ON sms_messages;
DROP POLICY IF EXISTS "Users can update SMS messages they are involved in" ON sms_messages;
DROP POLICY IF EXISTS "Users can delete SMS messages they are involved in" ON sms_messages;

-- Create simple policies that allow users to see messages where they are involved
